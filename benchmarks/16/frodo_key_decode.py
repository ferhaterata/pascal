...
folder_name = './benchmarks/16/'
file_name = 'frodo_key_decode.o'
proj = angr.Project(folder_name + file_name,
                    load_options={"auto_load_libs": False, 'main_opts': {'base_addr': base_addr}})
func_name = 'frodo_key_decode'
prototype = angr.types.parse_defns('void frodo_key_decode(uint16_t *out, const uint16_t *in);')[func_name]

out = [claripy.BVS('out!{}'.format(i), 16, explicit_name=True) for i in range(64)]
out_symbolic = claripy.Concat(*out)

in_ = [claripy.BVS('in!{}'.format(i), 16, explicit_name=True).annotate(Sensitive()) for i in range(16)]
...
for loop in proj.analyses.LoopFinder().loops:
    ...
    if hex(loop.entry.addr) in ('0x80001c8', '0x80001b0', '0x8000140'):
        loops.append(loop)
