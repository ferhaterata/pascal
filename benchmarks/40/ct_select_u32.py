...
folder_name = './benchmarks/40/'
file_name = 'ct_select_u32.o'
proj = angr.Project(folder_name + file_name,
                    load_options={"auto_load_libs": False, 'main_opts': {'base_addr': base_addr}})
func_name = 'ct_select_u32'
prototype = angr.types.parse_defns('uint32_t ct_select_u32(uint32_t x, uint32_t y, uint32_t bit);')[func_name]

x = claripy.BVS('x', 32, explicit_name=True).annotate(Sensitive())
y = claripy.BVS('y', 32, explicit_name=True).annotate(Sensitive())
bit = claripy.BVS('bit', 32, explicit_name=True).annotate(Sensitive())

addr = sca.getAddressOfSymbol(proj, func_name)
state: angr.SimState = proj.factory.call_state(addr, x, y, bit, prototype=prototype)
